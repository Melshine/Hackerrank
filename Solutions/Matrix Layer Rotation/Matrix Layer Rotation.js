...
function matrixRotation(matrix, r) {
    ...
    var rotate = Array(m).fill().map(_ => Array(n).fill());
    var frame_c_max = Math.min(m,n)/2;
    var list = Array(frame_c_max).fill().map(_ => []);
    ...
    function param(i,j){
        ...
        var frame_v = Math.min(ii,jj);
        var frame_c = 2 * (m+n-2) - 8 * frame_v;
